# Chanjet T+Cloud MCP Server

独立 Python MCP 服务，用于把畅捷通 T+Cloud OpenAPI 文档和业务接口暴露给 MCP 客户端。
...
`CHANJET_OPEN_TOKEN` 是调用 T+ 业务接口时请求头里的 `openToken`。如果只使用文档查询工具，可以暂时不配置业务接口凭据。
...
```json
{
  "mcpServers": {
    "chanjet-tcloud": {
      "command": "/Users/sun/Documents/Cursor/SP/.venv/bin/chanjet-tcloud-mcp",
